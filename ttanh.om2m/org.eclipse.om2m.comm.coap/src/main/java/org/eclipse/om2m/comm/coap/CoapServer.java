...
class CoapMessageDeliverer implements MessageDeliverer {
    ...
    private static String context = System.getProperty("org.eclipse.om2m.cseBaseContext","/om2m");
    Request req;
    Response resp;
   ...
   /**
    * Converts a {@link CoapServerRequest} to a {@link RequestIndication} and uses it to invoke the CSE service.
   ...
   public Response service (Request request) throws SocketException, IOException {
       ...
       //sending the standard request to the Cse and getting a standard response
       if(cse!=null){
           responseConfirm = cse.doRequest(requestIndication);
           LOGGER.info("check point: requestIndication sent and waiting for responseConfirm");
       }else{
           responseConfirm = new ResponseConfirm(StatusCode.STATUS_SERVICE_UNAVAILABLE, "CSE service not installed");
       }
