...
public class Switchs {
    ...
    public static void postGroups(String aPoCPath, String type, String location, List<String> keys) {
        ...
        for (int i=0; i<keys.size(); i++) {
            membersON.getReference().add(SampleMonitor.CSEID+"/applications/"+keys.get(i)+"/"+aPoCPath+"/true");
            membersOFF.getReference().add(SampleMonitor.CSEID+"/applications/"+keys.get(i)+"/"+aPoCPath+"/false");
        }
        groupON.setMembers(membersON);
        groupOFF.setMembers(membersOFF);

        // Groups Creation Request
        SampleMonitor.CSE.doRequest(new RequestIndication(METHOD,SampleMonitor.CSEID+"/groups",SampleMonitor.REQENTITY,groupON));
        SampleMonitor.CSE.doRequest(new RequestIndication(METHOD,SampleMonitor.CSEID+"/groups",SampleMonitor.REQENTITY,groupOFF));

        // Application Creation
        ResponseConfirm response = SampleMonitor.CSE.doRequest(new RequestIndication("CREATE",SampleMonitor.CSEID+"/applications",SampleMonitor.REQENTITY,new Application(APP_ID)));
        if (response.getStatusCode().equals(StatusCode.STATUS_CREATED)) {
            // Create DESCRIPTOR container sub-resource
            SampleMonitor.CSE.doRequest(new RequestIndication(METHOD,SampleMonitor.CSEID+"/applications/"+APP_ID+"/containers",SampleMonitor.REQENTITY,new Container(SampleMonitor.DESC)));
            // Create DESCRIPTION contentInstance on the DESCRIPTOR container resource
            String content = getDescriptorRep(SampleMonitor.CSEID, APP_ID, type, location, SampleMonitor.DESC);
            String targetID= SampleMonitor.CSEID+"/applications/"+APP_ID+"/containers/"+SampleMonitor.DESC+"/contentInstances";
            SampleMonitor.CSE.doRequest(new RequestIndication(METHOD,targetID,SampleMonitor.REQENTITY,new ContentInstance(content.getBytes())));
        }
    }
     ...
     */
    public static String getDescriptorRep(String cseId, String appId, String type, String location, String stateCont) {
        ...
        opON.setHref(new Uri(cseId+"/groups/"+GROUP_ON+"/membersContent"));
        ...
        opOFF.setHref(new Uri(cseId+"/groups/"+GROUP_OFF+"/membersContent"));
    ...
    public static void switchLamp(String appId, boolean newState) {
        SampleMonitor.CSE.doRequest(new RequestIndication("EXECUTE",SampleMonitor.CSEID+"/applications/"+appId+"/"+Lamp.APOCPATH+"/"+newState,SampleMonitor.REQENTITY, ""));
    ...
    public static void switchAll(boolean newState) {
        if(newState) {
            ...
            SampleMonitor.CSE.doRequest(new RequestIndication("EXECUTE",SampleMonitor.CSEID+"/groups/"+GROUP_ON+"/membersContent",SampleMonitor.REQENTITY, ""));
        } else {
            // Sends a request to groupOFF to switchAll lamps OFF
            SampleMonitor.CSE.doRequest(new RequestIndication("EXECUTE",SampleMonitor.CSEID+"/groups/"+GROUP_OFF+"/membersContent",SampleMonitor.REQENTITY, ""));
        }
