...
public class Notifier {
	...
	public static void performVerificationRequest(RequestPrimitive request,
		...
		for(String uri : subscriptionEntity.getNotificationURI()){
			if(!uri.equals(request.getFrom())){
				...
				LOGGER.info("\n======NOTIFIER========\n" 
							+ "\n notiRequest" + notifRequest.toString() 
							+ "\n uri" + uri
						);
	...
	static class NotificationWorker implements Runnable {
		...
		public void run() {
			...
			LOGGER.info("\n =======================Debugger Notifier=========================== \n" 
					+ "BigInteger.valueOf(resourceStatus) =="
					+  BigInteger.valueOf(resourceStatus));
			
			LOGGER.info("\n =======================Debugger Notification Woker=========================== \n" 
					+ "sub == "
					+  sub
					+ "\n" + sub.toString()
					);
			...
			LOGGER.info("\n =======================Debugger Notifier=========================== \n" 
					+ "sub.getNotificationContentType() =="
					+ sub.getNotificationContentType());
			...
			LOGGER.info("\n =======================Debugger Notifier=========================== \n" 
					...
					+notification.toString()
					+"\nsub.getNotificationPayloadContentType()== "
					+sub.getNotificationPayloadContentType()
					+ "\nsub.getNotificationURI()== "
					+ sub.getNotificationURI()
			...
			//sub.getNotificationPayloadContentType() =application/json | applicatin/obj
			//notification = org.eclipse.om2m.commons.resource.Notification@3a0f24ff
			//notification ==org.eclipse.om2m.commons.resource.Notification@4eb58590
			
			
			for (Entry<String, DataMapperService> s:DataMapperSelector.getDataMapperList().entrySet()){
				LOGGER.info("\n==========XEM TRONG DATA_MAPPER_SERVICE============\n");
				
				LOGGER.info(s.getKey() + "=" + s.getValue() 
						+ "\n" + s.getValue().toString()
						+ "\n" + s.getValue().getServiceDataType()
						);
			}
			
			
			//DataMapperSelector.getDataMapperList().get(sub.getNotificationPayloadContentType()).objToString(notification);
			// Set the content
			/**
			 * Doan nay bat dau sua de chap nhan obj
			 * */
			
			/*
			if (!MimeMediaType.OBJ.equals(sub.getNotificationPayloadContentType())) {
				...
				request.setContent(notification);
				request.setReturnContentType(sub.getNotificationPayloadContentType());
